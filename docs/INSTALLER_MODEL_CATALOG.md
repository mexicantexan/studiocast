...
No authoritative artifact byte sizes are present in the repository today. The
catalog records each one as `size_bytes: null` / `size_status: unknown` and its
size metadata as untrusted. Recommended mode therefore returns the blocker
`artifact_sizes_untrusted` before creating a destination or downloading data.
A signed stable release manifest must provide and authenticate authoritative
sizes before the release catalog can mark size metadata trusted. Sizes must
never be estimated or invented.
