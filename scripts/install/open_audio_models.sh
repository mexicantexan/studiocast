...
DEFAULT_DEST="${XDG_DATA_HOME:-${HOME}/.local/share}/studiocast/models/open_audio"
...
Options:
  --dest <path>       Open Audio model root (default: ${DEFAULT_DEST})
  --model <id>        Install one curated pack; repeatable.
  --force             Re-stage the selected curated pack.
  --list              List curated Open Audio packs and exit.
...
Legacy source environment variables remain supported. STUDIOCAST_MODEL_SOURCE
takes precedence; otherwise STUDIOCAST_HF_REPO_ID and STUDIOCAST_HF_REV form the
Hugging Face base URL. Maxine presence never removes these open-source defaults.
EOF
}
...
if [[ "${list}" -eq 1 ]]; then
  [[ "${json}" -eq 1 ]] || echo "Curated Open Audio packs:"
  args=(--catalog "${CATALOG}" --repo-root "${REPO_ROOT}" list --family open_audio)
  [[ "${json}" -eq 1 ]] && args+=(--json)
...
args=(--catalog "${CATALOG}" --repo-root "${REPO_ROOT}" install --family open_audio --dest "${dest}" --cache "${cache}" --retries "${retries}")
